...
<?php 
  ...
  $kosul = $db->prepare("SELECT * FROM chat_engelleme WHERE engel_atan=? AND engel_yiyen=? ");
   $kosul->execute(array($_SESSION['k_adi'],test_input($_POST['k_adi_text'])));

  $kosul2 = $db->prepare("SELECT * FROM chat_engelleme WHERE engel_yiyen=? AND engel_atan=? ");
